...
from cms_myblog.cms_appconfig import BlogConfig
#----------------------------- utils -----------------------------#


IMAGE_THUMBNAIL_SIZE = {"size": "120x120", "crop": True, "upscale": False}
IMAGE_FULL_SIZE = {"size": "640", "crop": True, "upscale": False}
...
USE_FALLBACK_LANGUAGE_IN_URL=False
...
def _get_language(instance, language):
    ...
    if language and language in available_languages:
        return language
    if USE_FALLBACK_LANGUAGE_IN_URL:
...
class BlogCategory(BlogMetaMixin, TranslatableModel):
    ...
    app_config = AppHookConfigField(BlogConfig, null=True, verbose_name=_("app. config"), related_name="app_config")
...
class Author(BlogMetaMixin, TranslatableModel):
    app_config = AppHookConfigField(BlogConfig, null=True, verbose_name=_("app. config"), related_name="author_app_config")

    translations = TranslatedFields(
        name=models.CharField(_("name"), max_length=752),
        description=models.CharField(max_length=234, blank=True, null=True),
...
class Post(KnockerModel, BlogMetaMixin, TranslatableModel):
    ...
    Blog post
    """
    #author=models.ForeignKey(Author, on_delete=models.PROTECT, null=True, blank=True, related_name="post_author")
    ...
    main_image_thumbnail = models.ForeignKey(
        ...
        related_name="post_image_thumbnail",
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
    )
    main_image_full = models.ForeignKey(
        ...
        related_name="post_main_image_full",
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
    )
    ...
    app_config = AppHookConfigField(BlogConfig, null=True, verbose_name=_("app. config"), related_name="post_app_config")
    ...
    media = PlaceholderField("media", related_name="post_media")
    content = PlaceholderField("post_content", related_name="post_contents")
    liveblog = PlaceholderField("live_blog", related_name="post_live_blog")
    ...
    def get_absolute_url(self, lang=None):
        ...
        with switch_language(self, lang):
            ...
            urlconf = PERMALINKS_URLS[self.app_config.url_patterns]
    ...
    def get_cache_key(self, language, prefix):
        return "myblog:{2}:{0}:{1}".format(language, self.guid, prefix)
...
class BasePostPlugin(CMSPlugin):
    app_config = AppHookConfigField(BlogConfig, null=True, verbose_name=_("app. config"), blank=True, related_name="basePostPlugin_app_conf")
...
class LatestPostsPlugin(BasePostPlugin):
    ...
    app_config = AppHookConfigField(BlogConfig, null=True, verbose_name=_("app. config"), blank=True, related_name="latest_basePostPlugin_app_conf")

    tags = TaggableManager(
        _("filter by tag"),
        blank=True,
        help_text=_("Show only the blog articles tagged with chosen tags."),
        related_name="latest_post_tags",
    )
    categories = models.ManyToManyField(
        "myblog.BlogCategory",
        blank=True,
