...
from myblog.forms import AuthorPostsForm, BlogPluginForm, LatestEntriesForm
from myblog.models import AuthorEntriesPlugin, BlogCategory, GenericBlogPlugin, LatestPostsPlugin, Post
...
class BlogPlugin(CMSPluginBase):
    ...
    def get_render_template(self, context, instance, placeholder):
        ...
        """
        templates = [os.path.join("myblog", instance.template_folder, self.base_render_template)]
        ...
        return selected.template.name #authors: myblog/plugins/archive.html
...
class BlogLatestEntriesPlugin(BlogPlugin):
    ...
    def get_fields(self, request, obj=None):
        ...
        """
        fields = ["app_config", "latest_posts", "authors"]
...
class BlogAuthorPostsPlugin(BlogPlugin):
    ...
    module = get_setting("PLUGIN_MODULE_NAME") #_("Blog")
    name = get_setting("AUTHOR_POSTS_PLUGIN_NAME")#_("Author Blog Articles")
    ...
    def get_fields(self, request, obj=None):
        ...
        'template_folder' field is added if ``BLOG_PLUGIN_TEMPLATE_FOLDERS`` contains multiple folders.
        """
        fields = ["app_config", "current_site", 'authors']#from AuthorEntriesPlugin
        if len(get_setting("PLUGIN_TEMPLATE_FOLDERS")) > 1:
            fields.append("template_folder")
        return fields#['app_config', 'current_site', 'authors']
...
class BlogAuthorPostsListPlugin(BlogAuthorPostsPlugin):
    ...
    name = get_setting("AUTHOR_POSTS_LIST_PLUGIN_NAME")#_("Author Blog Articles List")
