...
from myblog.forms import CategoryAdminForm, PostAdminForm
from .cms_appconfig import BlogConfig
from myblog.models import Post , BlogCategory#, Author


MULTISITE = True
LIVEBLOG_PLUGINS = ("LiveblogPlugin",)
USE_ABSTRACT = True
USE_PLACEHOLDER = True
USE_RELATED = True
ADMIN_POST_FIELDSET_FILTER = False
#------------------------------ utils -------------------------------#
...
#donnees d'entree (content du blog)
class BlogCategoryAdmin(ModelAppHookConfig, TranslatableAdmin):
    ...
    class Media:
        css = {"all": ("{}myblog/css/{}".format(settings.STATIC_URL, "myblog_blog_admin.css"),)}
...
class PostAdmin(PlaceholderAdminMixin, FrontendEditableAdminMixin, ModelAppHookConfig, TranslatableAdmin):
    ...
    inlines = []
    if apps.is_installed("myblog.liveblog"):
    ...
    def post_add_plugin(self, request, obj1, obj2=None):
        if isinstance(obj1, CMSPlugin):
            ...
            plugin = obj2
        if plugin.plugin_type in LIVEBLOG_PLUGINS:
    ...
    def publish_post(self, request, pk):
        ...
        try:
            ...
            try:
                ...
                return HttpResponseRedirect(reverse("myblog:posts-latest"))
